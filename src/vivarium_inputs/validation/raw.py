...
def validate_population_attributable_fraction(data: pd.DataFrame, entity: Union[RiskFactor, Etiology],
    ...
    for c_id in data.cause_id:
        cause = [c for c in causes if c.gbd_id == c_id][0]
        if cause.restrictions.yld_only and np.any((data.cause_id==c_id) & (data.measure_id == MEASURES['YLLs'])):
        ...
        if cause.restrictions.yll_only and np.any((data.cause_id==cid) & (data.measure_id == MEASURES['YLDs'])):
