...
def _validate_relative_risk(data: pd.DataFrame, entity: Union[RiskFactor, CoverageGap], location_id: int):
    ...
    # We have to loose this check to warning because of many risks have ab
...
def check_age_groups_paf(pafs: pd.DataFrame, relative_risk: pd.DataFrame):
    ...
    if set(rr.age_group_id) > set(pafs.age_group_id) and \
            not np.allclose(rr[~rr.age_group_id.isin(set(pafs.age_group_id))].value, 1.0):
