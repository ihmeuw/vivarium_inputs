...
def get_disability_weight(
    ...
    if entity.kind == "cause":
        ...
        try:
            ...
            data = utilities.normalize(data, DRAW_COLUMNS)
...
def get_remission_rate(
    ...
    data = utilities.normalize(data, DRAW_COLUMNS, fill_value=0)
    data = data.filter(DEMOGRAPHIC_COLUMNS + DRAW_COLUMNS)
    return data
...
def get_exposure(
    ...
    if entity.distribution in ["dichotomous", "ordered_polytomous", "unordered_polytomous"]:
        ...
        data = pd.concat(
            [
                utilities.normalize(exposed, DRAW_COLUMNS, fill_value=0),
                utilities.normalize(unexposed, DRAW_COLUMNS, fill_value=1),
            ],
            ignore_index=True,
        )
...
def get_relative_risk(
    ...
    data = (
        ...
        .apply(utilities.normalize, cols_to_fill=DRAW_COLUMNS, fill_value=1)
...
def get_population_attributable_fraction(
    ...
    data = (
        ...
        .apply(utilities.normalize, cols_to_fill=DRAW_COLUMNS, fill_value=0)
        .reset_index(drop=True)
    )
    data = data.filter(
...
def get_demographic_dimensions(
    ...
    demographic_dimensions = utilities.normalize(demographic_dimensions, DRAW_COLUMNS)
