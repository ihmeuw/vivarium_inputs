...
def get_live_births_by_sex(override_config=None):
    config = get_input_config(override_config)
    data = core.get_covariate_estimate(covariates.live_births_by_sex, config.input_data.location)
...
def get_dtp3_coverage(override_config=None):
    config = get_input_config(override_config)
    data = core.get_covariate_estimate(covariates.dtp3_coverage_proportion, config.input_data.location)
    data = normalize_for_simulation(data)
