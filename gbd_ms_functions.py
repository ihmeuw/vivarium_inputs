...
def get_modelable_entity_draws(location_id, year_start, year_end, measure, me_id):
    ...
    source_draws = source_draws.query('year_id>={ys} and year_id<={ye}'.format(ys=year_start, ye=year_end)).copy()

    source_draws = get_age_from_age_group_id(source_draws)

    # For now, do not include information on early, pre, and post neonatal
    source_draws = source_draws.query("age != 0")
    ...
    source_draws = source_draws.set_index(['year_id','age']).sortlevel()

    for sex_id in (1,2):


        draws = source_draws.query("sex_id == {s}".format(s=sex_id))
...
def get_sbp_mean_sd(location_id, year_start, year_end, draw_number):
    ...
    for sex_id in [1,2]:
        ...
        # Keep only draw columns
        keepcol = ['exp_mean_{i}'.format(i=draw_number)]
        sdcol = ['exp_sd_{i}'.format(i=draw_number)]
