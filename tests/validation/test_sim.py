...
def mock_validation_context():
    ...
    context = sim.SimulationValidationContext(
        location="United States", years=years, age_bins=age_bins
    )
...
def test_validate_location_column_fail(mock_validation_context, locations, expected_location):
    ...
    df = pd.DataFrame({"location": locations, "other": [0] * len(locations)}).set_index(
        ["location", "other"]
    )
...
def test_validate_sex_column_pass():
    df = pd.DataFrame({"sex": ["Male", "Female"], "other": [0] * 2}).set_index(
        ["sex", "other"]
    )
...
def test_validate_age_columns_pass(mock_validation_context):
    ages = mock_validation_context["age_bins"].filter(["age_start", "age_end"])
    # Shuffle the rows and set index
    ages = ages.sample(frac=1).reset_index(drop=True)
    ages = ages.set_index(
        pd.IntervalIndex.from_arrays(ages.age_start, ages.age_end, closed="left", name="age"),
        append=True,
    )
...
def test_validate_age_columns_invalid_age(mock_validation_context):
    ages = mock_validation_context["age_bins"].filter(["age_start", "age_end"]).copy()
...
def test_validate_year_columns_pass(mock_validation_context):
    ...
    years = years.set_index(
        pd.IntervalIndex.from_arrays(
            years.year_start, years.year_end, closed="left", name="year"
        ),
        append=True,
    )
...
@pytest.mark.parametrize(
    "values", [(1, 2, np.inf), (1, np.nan, 2)], ids=["infinity", "missing"]
)
...
@pytest.mark.parametrize(
    "values, ids, restriction_type, fill",
    [
...
def test_check_age_restrictions(
    ...
    idx = pd.IntervalIndex.from_arrays(
        age_bins.age_start, age_bins.age_end, closed="left", name="age"
    )
...
def test_check_age_restrictions_fail(
    ...
    df = pd.DataFrame({"value": values}, index=idx)
    with pytest.raises(DataTransformationError):
        sim.check_age_restrictions(
            df, entity, restriction_type, fill, mock_validation_context
        )


@pytest.mark.parametrize(
    "values, restrictions, fill",
    [
...
def test_check_sex_restrictions(values, restrictions, fill):
    df = pd.DataFrame(
        {"sex": ["Male", "Male", "Female", "Female"], "other": [0] * 4, "value": values}
    )
...
def test_validate_expected_columns_fail(df):
    ...
    with pytest.raises(DataTransformationError, match="missing columns"):
        sim.validate_expected_index_and_columns(
            [None], df.index.names, cols + ["e"], df.columns
        )
    with pytest.raises(DataTransformationError, match="missing index names"):
        data = df.set_index(cols[:2])
        sim.validate_expected_index_and_columns(
            cols[:3], data.index.names, cols[:2], data.columns
        )
    ...
    with pytest.raises(DataTransformationError, match="extra index names"):
        data = df.set_index(cols[:2])
        sim.validate_expected_index_and_columns(
            cols[:1], data.index.names, cols[:2], data.columns
        )
