...
@pytest.mark.parametrize(
    "mort, morb, yld_only, yll_only, match",
    [
...
def test_check_data_exist_always_fail(data):
    ...
    assert (
        raw.check_data_exist(
            data, value_columns=data.columns, zeros_missing=False, error=False
        )
        is False
    )
    ...
    assert (
        raw.check_data_exist(
            data, value_columns=data.columns, zeros_missing=True, error=False
        )
        is False
    )
...
def test_check_data_exist_value_columns():
    ...
    assert raw.check_data_exist(
        df, value_columns=["a", "b", "c"], zeros_missing=True, error=False
    )

    assert (
        raw.check_data_exist(df, value_columns=["a", "b"], zeros_missing=True, error=False)
        is False
    )
...
@pytest.mark.parametrize(
    "sex_ids, male, female, both",
    [
        ([1, 2, 12], True, False, False),
...
@pytest.mark.parametrize(
    "sex_ids, male, female, both, warn_extra, warn_missing",
    [
...
def test_check_sex_ids_warn(
    mock_validation_context, caplog, sex_ids, male, female, both, warn_extra, warn_missing
):
    df = pd.DataFrame({"sex_id": sex_ids})
    raw.check_sex_ids(df, mock_validation_context, male, female, both)
    assert warn_extra == (
        "extra" in caplog.text
    ), "The expected warning message was not raised for extra sex ids"
    assert warn_missing == (
        "missing" in caplog.text
    ), "The expected warning message was not raised for missing sex ids"
...
def test_check_age_restrictions_fail(
    mock_validation_context, data, start, end, val_cols, match
):
    with pytest.raises(DataAbnormalError, match=match):
...
test_data = [
    (
        pd.DataFrame({"sex_id": [1, 1], "a": 0, "b": 0, "c": 1}),
        True,
        False,
        ["a", "b"],
        "missing data values for males",
    ),
    (
        pd.DataFrame({"sex_id": [2, 2, 2, 3], "a": 0, "b": 0}),
        False,
        True,
        ["a", "b"],
        "missing data values for females",
    ),
    (
        pd.DataFrame({"sex_id": [3], "a": [0], "b": [0]}),
        False,
        False,
        ["a", "b"],
        "values for both",
    ...
    (
        pd.DataFrame({"sex_id": [1, 2], "a": [0, 1], "b": [0, 1]}),
        False,
        False,
        ["a", "b"],
        "values for both",
...
def test_check_sex_restrictions_fail(
    mock_validation_context, data, male, female, val_cols, match
):
    with pytest.raises(DataAbnormalError, match=match):
        raw.check_sex_restrictions(
            data, mock_validation_context, male, female, value_columns=val_cols
        )


test_data = [
    (
        pd.DataFrame({"sex_id": [1, 2], "a": 1, "b": 0}),
        True,
        False,
        ["a", "b"],
        "non-male sex ids",
...
@pytest.mark.parametrize("data, male, female, val_cols, match", test_data)
def test_check_sex_restrictions_warn(
    mock_validation_context, caplog, data, male, female, val_cols, match
):
    raw.check_sex_restrictions(
        data, mock_validation_context, male, female, value_columns=val_cols
    )
...
test_data = [
    (pd.DataFrame({"sex_id": [1, 1], "a": 1, "b": 0, "c": 0}), True, False, ["a", "b"]),
...
@pytest.mark.parametrize(
    "m_ids, expected, match",
    [([1, 1, 1, 12], ["A"], "multiple"), ([3, 3, 3], ["A", "B"], "not in the expected")],
)
...
@pytest.mark.parametrize("m_ids, expected", [([1], ["A"]), ([2, 2, 2, 2], ["A", "B"])])
def test_check_measure_id_pass(m_ids, expected, measures_mock):
    df = pd.DataFrame({"measure_id": m_ids})
    raw.check_measure_id(df, expected)


@pytest.mark.parametrize("m_ids, expected", [([1, 1, 1, 12], "A"), ([3, 3, 3], "A")])
...
@pytest.mark.parametrize("m_ids, expected", [([1], "A"), ([2, 2, 2, 2], "B")])
